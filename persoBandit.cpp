...
PersoBandit::PersoBandit(int level) :Personnage()
{
	m_persos.resize(1,NULL);
	m_persos[D1] = SDL_LoadBMP("images/bandit1.bmp");
...
PersoBandit::~PersoBandit()
{
	SDL_FreeSurface(m_affPoints);
	TTF_CloseFont(m_police);
}

int PersoBandit::getPoints()
{
	return m_points;
}

void PersoBandit::recevoirDegat(int degat)
{
...
void PersoBandit::afficher(SDL_Surface *ecran,int xscroll, int yscroll)
{
...
void PersoBandit::evoluer(const vector<vector<int> > &niveau,Personnage &perso,vector<Personnage*>& persos,int xscroll, int yscroll)
{
	int vx,vy=0;
...
void PersoBandit::recupererVecteur(int *vx, int *vy)
{
...
void PersoBandit::deplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
...
bool PersoBandit::essaieDeplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
	SDL_Rect test;
	test=m_rect;
	test.x += vx;
...
bool PersoBandit::collision(const vector<vector<int> > &niveau, SDL_Rect &test)
{
