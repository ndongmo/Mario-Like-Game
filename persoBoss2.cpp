...
PersoBoss2::PersoBoss2(int level) :Personnage()
{
	m_persos.resize(2,NULL);
	m_persos[D1] = SDL_LoadBMP("images/boss2G.bmp");
	m_persos[D2] = SDL_LoadBMP("images/boss2D.bmp");
...
PersoBoss2::~PersoBoss2()
{
	SDL_FreeSurface(m_affPoints);
	TTF_CloseFont(m_police);
}

int PersoBoss2::getPoints()
{
	return m_points;
}

void PersoBoss2::recevoirDegat(int degat)
{
	m_vies-=degat;
	if(!estVivant())
	{
...
void PersoBoss2::lancer(vector<Personnage*>& persos)
{
	persos.push_back(new Feu(m_toucheDActif,m_rect,m_level));
	m_temps=SDL_GetTicks();
}

void PersoBoss2::afficher(SDL_Surface *ecran,int xscroll, int yscroll)
{
...
void PersoBoss2::evoluer(const vector<vector<int> > &niveau,Personnage &perso,vector<Personnage*>& persos,int xscroll, int yscroll)
{
	int vx,vy=0;
...
void PersoBoss2::recupererVecteur(int *vx, int *vy,Personnage &perso)
{
	SDL_Rect pos;
	pos=perso.getRect();
...
void PersoBoss2::deplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
...
bool PersoBoss2::essaieDeplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
	SDL_Rect test;
	test=m_rect;
	test.x += vx;
...
bool PersoBoss2::collision(const vector<vector<int> > &niveau, SDL_Rect &test)
{
