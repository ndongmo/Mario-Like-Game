...
PersoChampignon::PersoChampignon() :Personnage()
{
	m_persos.resize(2,NULL);
	m_persos[D1] = SDL_LoadBMP("images/champignon.bmp");

	m_nom="PERSOCHAMPIGNON";
	m_vies=8;
	m_vitesseH=6;
...
PersoChampignon::~PersoChampignon()
{
}

int PersoChampignon::getPoints()
{
	return m_points;
}

void PersoChampignon::recevoirDegat(int degat)
{
	m_vies-=degat;
	m_transparent=0;
}

void PersoChampignon::afficher(SDL_Surface *ecran,int xscroll, int yscroll)
{
	SDL_Rect posPerso;
	posPerso=m_rect;
	posPerso.x-=xscroll;
...
void PersoChampignon::evoluer(const vector<vector<int> > &niveau,Personnage &perso,vector<Personnage*>& persos,int xscroll, int yscroll)
{
	int vx,vy=0;
...
void PersoChampignon::recupererVecteur(int *vx, int *vy,Personnage &perso)
{
	SDL_Rect pos;
	pos=perso.getRect();
...
void PersoChampignon::deplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
...
bool PersoChampignon::essaieDeplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
	SDL_Rect test;
	test=m_rect;
	test.x += vx;
...
bool PersoChampignon::collision(const vector<vector<int> > &niveau, SDL_Rect &test)
{
