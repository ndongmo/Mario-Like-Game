...
PersoTortue::PersoTortue(int level) :Personnage()
{
	m_persos.resize(2,NULL);
	m_persos[D1] = SDL_LoadBMP("images/tortueD.bmp");
	m_persos[D2] = SDL_LoadBMP("images/tortueG.bmp");
...
PersoTortue::~PersoTortue()
{
	SDL_FreeSurface(m_affPoints);
	TTF_CloseFont(m_police);
}

int PersoTortue::getPoints()
{
	return m_points;
}

void PersoTortue::recevoirDegat(int degat)
{
...
void PersoTortue::afficher(SDL_Surface *ecran,int xscroll, int yscroll)
{
...
void PersoTortue::evoluer(const vector<vector<int> > &niveau,Personnage &perso,vector<Personnage*>& persos,int xscroll, int yscroll)
{
	int vx,vy=0;
...
void PersoTortue::recupererVecteur(int *vx, int *vy)
{
...
void PersoTortue::deplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
...
bool PersoTortue::essaieDeplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
	SDL_Rect test;
	test=m_rect;
	test.x += vx;
...
bool PersoTortue::collision(const vector<vector<int> > &niveau, SDL_Rect &test)
{
