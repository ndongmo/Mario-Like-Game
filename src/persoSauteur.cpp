...
PersoSauteur::PersoSauteur(int level) :Personnage()
{
	m_persos.resize(2,NULL);
	m_persos[D1] = SDL_LoadBMP("images/sauteur1.bmp");
	m_persos[D2] = SDL_LoadBMP("images/sauteur2.bmp");
...
PersoSauteur::~PersoSauteur()
{
	SDL_FreeSurface(m_affPoints);
	TTF_CloseFont(m_police);
}

int PersoSauteur::getPoints()
{
	return m_points;
}

void PersoSauteur::recevoirDegat(int degat)
{
	m_vies-=degat;
	if(!estVivant())
	{
...
void PersoSauteur::afficher(SDL_Surface *ecran,int xscroll, int yscroll)
{
...
void PersoSauteur::evoluer(const vector<vector<int> > &niveau,Personnage &perso,vector<Personnage*>& persos,int xscroll, int yscroll)
{
	int vx,vy=0;
...
void PersoSauteur::recupererVecteur(int *vx, int *vy,Personnage &perso)
{
	SDL_Rect pos;
	pos=perso.getRect();
	if(m_vies > 2)
...
void PersoSauteur::deplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
...
bool PersoSauteur::essaieDeplacer(int vx,int vy,const vector<vector<int> > &niveau)
{
	SDL_Rect test;
	test=m_rect;
	test.x += vx;
...
bool PersoSauteur::collision(const vector<vector<int> > &niveau, SDL_Rect &test)
{
